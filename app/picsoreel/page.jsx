...
const Picsoreel = () => {
  ...
  return (
    <main className="bg-[url('/img/sponsor/background.png')] bg-cover bg-center  min-h-dvh px-3">
      <div className="max-w-sm lg:max-w-4xl mx-auto flex flex-col lg:gap-1 gap-2 justify-center items-center lg:py-14 py-8">
        ...
        {picsoreel && picsoreel.map((event, index) => (
          <motion.div
            key={event.id} // Ensure event has a unique id
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5, delay: index * 0.1 }} // Adds staggered animation for each card
